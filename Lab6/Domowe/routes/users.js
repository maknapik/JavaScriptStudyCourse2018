...
router.post('/logins/students', function(req, res, next) {
    Student.find({}, function(err, students) {
        ...
        if(logins) {
            ...
            if(req.body.filter == "true") logins = logins.filter(login => login.startsWith(req.body.login));
        }
        if(req.body.login.length < 1) logins = [];
        res.send(JSON.stringify(logins));
    });
});

router.post('/logins/teachers', function(req, res, next) {
...
router.get('/register', function(req, res, next) {
    Student.find({}, function(err, students) {
        ...
        Teacher.find({}, function(err, teachers) {
            ...
            console.log(logins);
            res.render('register', {page: 'register', title: 'Register', errors: null, logins: logins });
        })
    });
});
